...
//      executor.allowCoreThreadTimeOut(true);
  ...
  public static ExecutorService getMyPerThreadExecutor() {
    ...
    if (thread instanceof  SolrThread) {
      return getExecutorService("SolrPerThread", Integer.getInteger("solr.perThreadPoolSize", Math.max(SysStats.PROC_COUNT, 4)), false);
    }
  ...
  private static void handleObject(AtomicReference<Throwable> exception, final TimeTracker workUnitTracker, Object ob, boolean ignoreExceptions) {
    ...
    try {
      ...
      if (ignoreExceptions) {
        ...
        log.error("handleObject(AtomicReference<Throwable>=" + exception + ", CloseTimeTracker=" + workUnitTracker + ")" + ", Object=" + ob + ")", t);
  ...
  public static <K, V> Map<K, V> concMapSmallO() {
    return new NonBlockingHashMap<K, V>(132);
  }

  public static <K, V> Map<K, V> concMapReqsO() {
    return new NonBlockingHashMap<>(128);
  }

  public static <K, V> Map<K, V> concMapClassesO() {
    return new NonBlockingHashMap<>(132);
  }
