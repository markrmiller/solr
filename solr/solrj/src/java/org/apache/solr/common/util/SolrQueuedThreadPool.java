...
public class SolrQueuedThreadPool extends ContainerLifeCycle implements ThreadFactory, ThreadPool.SizedThreadPool, Dumpable, TryExecutor, Closeable {
  ...
  private final int _maxThreadsBeforeQueue;
  ...
  private SolrQueuedThreadPool(String name, @Name("maxThreads") int maxThreads, @Name("minThreads") int minThreads, @Name("idleTimeout") int idleTimeout, @Name("reservedThreads") int reservedThreads,
    ...
    if (maxThreads == Integer.MAX_VALUE) {
      _maxThreadsBeforeQueue = 768;
    } else {
      _maxThreadsBeforeQueue = maxThreads;
    }

    if (queue == null) {
      int capacity = Math.max(_minThreads, 64) * 8;
  ...
  public void execute(Runnable job) {
    ...
    int loops = 0;
    while (true) {
      log.trace("loops is {} queue  {} remaining capacity{}", loops++, getQueueSize(), getQueue().remainingCapacity());
      ...
      startThread = (idle <= 0 && threads < _maxThreadsBeforeQueue) ? 1 : 0;
    ...
    if (!_jobs.offer(job)) {
      if (_maxThreads == Integer.MAX_VALUE) {
        startThread = 1;
      } else {
  ...
  private void ensureThreads() {
    while (!closed) {
      ...
      int idle = AtomicBiInteger.getLo(counts);
      if (threads < _minThreads || (idle < 0 && threads < _maxThreadsBeforeQueue)) {
  ...
  protected void startThread() {
    log.trace("Start a thread {}", getName());
