...
public class NumberRangeHistogram extends Histogram {
	...
	final protected List<Bucket> cluster(final List<StatisticsEntry> entries) {
		...
		return topsAndBuckets.stream().map(bigIntegerBucketPair -> bigIntegerBucketPair._2).collect(Collectors.toList());
	}
	...
	private List<Pair<BigInteger, Bucket>> topsAndBuckets(final BigInteger min, final BigInteger max) {
