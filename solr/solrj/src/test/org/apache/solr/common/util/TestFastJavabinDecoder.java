...
public class TestFastJavabinDecoder extends SolrTestCaseJ4 {
  ...
  public void testTagRead() throws Exception {
    ...
    try (JavaBinCodec codec = new JavaBinCodec(faos, null, true)) {
      JavaBinCodec.writeVal(codec, 10);
      JavaBinCodec.writeVal(codec, 100);
      JavaBinCodec.writeVal(codec, "Hello!");
    }
    ...
    try (FastJavaBinDecoder.StreamCodec scodec = new FastJavaBinDecoder.StreamCodec(fis)) {
      ...
      assertEquals(10, JavaBinCodec.readSmallInt(scodec));
      tag = scodec.getTag();
      assertEquals(Tag._SINT, tag);
      assertEquals(100, JavaBinCodec.readSmallInt(scodec));
      tag = scodec.getTag();
      assertEquals(Tag._STR, tag);
      assertEquals("Hello!", JavaBinCodec.readStr(scodec));
    }
  }
  ...
  public void testParsingWithChildDocs() throws IOException {
    ...
    binaryResponseParser.processResponse(
        new FastInputStream(null, baos.toBytes(), 0, baos.size()), null);
