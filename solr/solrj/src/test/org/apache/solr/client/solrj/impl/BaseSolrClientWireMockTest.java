...
//})
@Ignore // needs updating, java.net.UnknownHostException: http: Temporary failure in name resolution
public abstract class BaseSolrClientWireMockTest extends SolrTestCase {
  ...
  static class JettyHttp2Server extends Jetty94HttpServer {
    ...
    protected Server createServer(Options options) {
      ...
      return new Server(new SolrQTP("JettyMockWireServer"));
    }
  }
...
//    qtp.start();
  ...
  protected static DocCollection buildMockDocCollection() throws IOException {
    ...
    return new DocCollection(BUILT_IN_MOCK_COLLECTION, Slice.loadAllFromMap(BUILT_IN_MOCK_COLLECTION, -1, slices), props, CompositeIdRouter.DEFAULT);
  }
  ...
  public void createTestClient() throws TimeoutException, InterruptedException, IOException {
    ...
    MockZkStateReader zkStateReader = new MockZkStateReader(clusterState, Collections.singleton(baseUrl));
