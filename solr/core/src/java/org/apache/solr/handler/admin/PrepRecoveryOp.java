  ...
  public void execute(CallInfo it) throws Exception {
    ...
    if (checkIsLeader) {
      ...
      if (leaderElector == null || !leaderElector.isLeader()) {
        log.debug("checkValidLeader failed for {} {} state={} params={}", leaderName, leaderElector, leaderElector == null ? "(null elector)" : leaderElector.getState(), params);
    ...
    try {
      log.debug("going to wait for core: {}, state: {}: params={}", cname, waitForState, params);
      ...
      coreContainer.getZkController().getZkStateReader().waitForState(collection, 15, TimeUnit.SECONDS, (n, c) -> {
        ...
        if (replica.getState() == finalWaitForState) {
          ...
          return true;
        } else if (replica.getState() == Replica.State.ACTIVE) { // if we look active, we have been getting updates
          return true;
        }
        return false;
      });
