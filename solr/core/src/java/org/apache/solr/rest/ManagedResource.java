...
public abstract class ManagedResource {
  ...
  protected final AtomicReference<NamedList<Object>> managedInitArgs = new AtomicReference<>();
  ...
  public void loadManagedDataAndNotify(Collection<ManagedResourceObserver> observers) 
    ...
    notifyObserversDuringInit(managedInitArgs.get(), observers);
  ...
  protected boolean updateInitArgs(NamedList<?> updatedArgs) {
    ...
    AtomicBoolean madeChanges = new AtomicBoolean(false);

    AtomicBoolean finalMadeChanges = madeChanges;
    managedInitArgs.getAndUpdate(entries -> {
      finalMadeChanges.set(!entries.equals(updatedArgs));
      if (finalMadeChanges.get()) {
        return (NamedList<Object>) updatedArgs.clone();
      } else {
        return entries;
      }
    });

    return madeChanges.get();
  }
  ...
  protected void reloadFromStorage() throws SolrException {
    ...
    NamedList<Object> newManagedInitArgs = managedInitArgs.updateAndGet(entries -> entries == null ? new NamedList<>() : entries);

    onManagedDataLoadedFromStorage(newManagedInitArgs, managedData);
  }
  ...
  protected Object processStoredData(Object data) throws SolrException {
    ...
    if (data != null) {
      ...
      managedInitArgs .set(new NamedList<>(initArgsMap));
  ...
  protected Map<String,Object> buildMapToStore(Object managedData) {
    ...
    toStore.put(INIT_ARGS_JSON_FIELD, convertNamedListToMap(managedInitArgs.get()));
