...
public class SolrDispatchFilter extends BaseSolrFilter {
  ...
  public final static boolean ASYNC = System.getProperty("solr.asyncDispatchFilter") == null||Boolean.getBoolean("solr.asyncDispatchFilter");
  public final static boolean ASYNC_IO = System.getProperty("solr.asyncIO") == null||Boolean.getBoolean("solr.asyncIO");
  ...
  static {
    log.warn("expected pre init of factories {} {} {} {} {} {} {}",
        ...
        ValueSourceParser.standardValueSourceParsers.getClass().getSimpleName(), CodepointCollator.getInstance());
  }
          ...
          if (ASYNC) {
            ...
            try {
              ...
              if (!servletResponse.isCommitted()) {
                sendException(e, null, servletResponse);
              }
            } finally {
...
//              break;
      ...
      if (!servletResponse.isCommitted() && !ASYNC) {
        ...
        servletResponse.sendError(code, e.getMessage());
      } else {
        exp = e;
      }
    } finally {
      ...
      try {
        ...
        //if (!ASYNC_IO && result == Action.PROCESS || result == Action.ADMIN||result == Action.RETURN) {
//      //   consumeInputFully(servletRequest, servletResponse);
        //}
        ...
        if (multiParts != null && multiParts.getContext() == servletRequest.getServletContext() && !multiParts.isEmpty()) {
          ParWork.submitIO("multipartCleanup", () -> {
            SolrRequestParsers.getDefaultInstance().cleanupMultipartFiles(multiParts);
          });
        }
      } finally {
  ...
  private void sendException(Exception exp, SolrCall call, HttpServletResponse response) throws IOException {
...
//    }
  ...
  private static class SolrAsyncListener implements AsyncListener
  {
    ...
    {
      log.info("onComplete {}", event, event.getThrowable());
    }
