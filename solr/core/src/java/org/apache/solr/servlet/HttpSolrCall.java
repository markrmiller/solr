...
public class HttpSolrCall extends SolrCall {
  ...
  public HttpSolrCall(SolrDispatchFilter solrDispatchFilter, String path, HttpServletRequest request, HttpServletResponse response) throws Exception {
    ...
    if (reqHandler != null) {
      solrRequest = SolrRequestParsers.DEFAULT.parse(null, reqPath, req);
    ...
    if (idx > -1) {
      ...
      if (idx2 > 0) {
        // save the portion after the ':' for a 'handler' path parameter
        origCorename = reqPath.substring(1, idx2);
      } else {
        origCorename = reqPath.substring(1);
      }
      ...
      if (solrCore == null && log.isDebugEnabled()) {
        log.debug("tried to get core by name {} got {}, existing cores {} found={}", origCorename, solrCore, cores.getAllCoreNames(), cores.getLoadedCoreNames());
      }
    ...
    if (solrCore == null && cores.isZooKeeperAware()) {
      ...
      String def = solrCore != null ? solrCore.getCoreDescriptor().getCollectionName() : origCorename;
    ...
    if (solrCore != null) {
      ...
      if (log.isTraceEnabled()) {
        log.trace("Extract handler from url path {}", reqPath);
      }

      if (reqHandler == null && reqPath.length() > 1) { // don't match "" or "/" as valid path
        reqHandler = solrCore.getRequestHandler(reqPath);
        if (log.isTraceEnabled()) {
          log.trace("handler={} name={}", solrCore, reqPath);
        }
        ...
        if (reqHandler == null && parser.isHandleSelect()) {
          if ("/select".equals(reqPath) || "/select/".equals(reqPath)) {
            solrRequest = parser.parse(solrCore, reqPath, req);
            String qt = solrRequest.getParams().get(CommonParams.QT);
      ...
      if (reqHandler != null) {
        ...
        this.solrReq = parser.parse(solrCore, reqPath, req);
  ...
  protected Action autoCreateSystemColl(String corename) throws Exception {
  ...
  public Action call() throws IOException {
    ...
    if (cores == null) {
      sendError(503, "Server is shutting down or failed to initialize", response);
      return RETURN;
    }
    ...
    try {
      ...
      switch (action) {
        ...
        case REMOTEQUERY:
          ...
          Action a = remoteQuery(req, response, coreUrl + path, solrDispatchFilter.getHttpClient());
          return a;
        case PROCESS:
