...
public abstract class SolrCall {
  ...
  protected SolrCore getCoreByCollection(CoreContainer cores, String collectionName, boolean isPreferLeader, HttpServletRequest request) {
  ...
  private SolrCore randomlyGetSolrCore(DocCollection collection, CoreContainer cores, List<Replica> replicas, boolean checkActive) {
  ...
  protected static String getRemoteCoreUrl(CoreContainer cores, String collectionName) throws SolrException {
    log.debug("getRemoteCoreUrl for {}", collectionName);
  ...
  SolrDispatchFilter.Action remoteQuery(HttpServletRequest req, HttpServletResponse response, String coreUrl, HttpClient httpClient) throws IOException {
    if (req != null) {
      ...
      if (hasContent(req)) {
        InputStreamContentProvider defferedContent = new InputStreamContentProvider(req.getInputStream(), 8192, true);
        proxyRequest.content(defferedContent);
      }
      AtomicReference<Throwable> failException = new AtomicReference<>();

      proxyRequest.onResponseHeaders(new RemoteInputStreamResponseListener(req, response)).send(new Response.Listener() {
  ...
  private static class RemoteInputStreamResponseListener extends InputStreamResponseListener {
    ...
    public RemoteInputStreamResponseListener(HttpServletRequest httpServletRequest, HttpServletResponse httpServletResponse) {
