...
public class ZkController implements Closeable, Runnable {
  ...
  public final ExecutorService zkRegisterExecutor = ParWork.getExecutorService("zkRegisterExecutor", 32);
...
//  };
  ...
  private final Map<String, ReplicateFromLeader> replicateFromLeaders = new ConcurrentHashMap<>(16, 0.75f);
  private final Map<String, ZkCollectionTerms> collectionToTerms = new ConcurrentHashMap<>(16, 0.75f);
  ...
  public void close() {
    ...
    try (ParWork closer = new ParWork(this, true)) {
      ...
      collectionToTerms.forEach((s, zkCollectionTerms) -> {
        zkCollectionTerms.disableRemoveWatches();
      ...
      if (zkStateReader != null) {
        ...
        // we are shutting it all down, no need to run through and remove all watches
        zkStateReader.disableRemoveWatches();
      }
  ...
  private String register(String coreName, final CoreDescriptor desc, boolean afterExpiration) {
   ...
   // LeaderElector leaderElector = null;
    ...
    try {
      ...
      log.debug("Register replica - core={} id={} address={} collection={} shard={} type={}", coreName, desc.getCoreProperty("id", null), baseUrl, collection, shardId, cloudDesc.getReplicaType());
      ...
      for (int i = 0; i < 30; i++) {
        ...
        try {
          ...
          Replica leader = zkStateReader
              .getLeaderRetry(getCoreContainer().getUpdateShardHandler().getLeaderCheckClient(),
                  collection, shardId, Integer.getInteger("solr.getleader.looptimeout", 5000), false);
...
//        }
  ...
  public Replica getLeaderProps(final String collection, long id, final String slice, int timeoutms, boolean failImmediatelyOnExpiration)
    ...
    try {
      ...
      return new Replica(leaderProps.getNodeProps().getStr(CORE_NAME_PROP), leaderProps.getNodeProps(), collection, id, slice);
  ...
  private void joinElection(CoreDescriptor cd, boolean joinAtHead) {
    ...
    Replica replica = new Replica(cd.getName(), props, collection, Long.parseLong(collId), shardId);
    ...
    leaderElector = leaderElectors.computeIfAbsent(replica.getName(), e ->  new LeaderElector(this));
...
//      }
  ...
  public static void linkConfSet(SolrZkClient zkClient, String collection, String confSetName) throws KeeperException, InterruptedException {
    ...
    if (data != null) {
      ...
      Map<String, Object> newProps = new HashMap<>(props);
  ...
  private void unregisterConfListener(String confDir, Runnable listener) {
    boolean removeWatch = false;
    synchronized (confDirectoryListeners) { // MRM TODO: remove
      ...
      if (confListeners.confDirListeners.isEmpty()) {
        if (confDirectoryListeners.remove(confDir) != null) {
          removeWatch = true;
        }
    ...
    if (removeWatch) {
      try {
        zkClient.removeAllWatches(confDir);
