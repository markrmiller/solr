...
public class LeaderElector implements Closeable {
  ...
  private boolean checkIfIamLeader(final ElectionContext context, boolean replacement) throws InterruptedException {
    try {
      ...
      log.debug(context.replica.getSlice() + " already closed", e);
      return false;
    } catch (KeeperException.SessionExpiredException e) {
      log.warn(context.replica.getSlice() + " session expired", e);
      return false;
    } catch (Exception e) {
      log.error(context.replica.getSlice() + " failed in checkIfIamLeader", e);
      return true;
  ...
  public void doJoinElection(ElectionContext context, boolean replacement, boolean joinAtHead) throws KeeperException, InterruptedException {
    ...
    while (true) {
      try {
        ...
        log.error(context.replica.getSlice() + "{} No leader election node found", e);
        break;
      }
    }
