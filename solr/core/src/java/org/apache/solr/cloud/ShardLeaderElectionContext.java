...
final class ShardLeaderElectionContext extends ShardLeaderElectionContextBase {
  ...
  synchronized boolean runLeaderProcess(ElectionContext context, boolean weAreReplacement, int pauseBeforeStart) throws KeeperException,
    ...
    log.debug("Run leader process for shard [{}] election, first step is to try and sync with the shard", context.leaderProps.getSlice());
    try (SolrCore core = cc.getCore(coreName)) {
      ...
      log.debug("Sync strategy sync result {}", result);
      ...
      ZkNodeProps zkNodes = ZkNodeProps
          .fromKeyVals(StatePublisher.OPERATION, OverseerAction.STATE.toLower(), ZkStateReader.COLLECTION_PROP, collection, ZkStateReader.CORE_NAME_PROP, leaderProps.getName(), "id",
              cd.getCoreProperty("collId", null)+ "-" + cd.getCoreProperty("id", null), ZkStateReader.STATE_PROP, "leader");

      if (cc.isShutDown() || cc.getZkController().isDcCalled()) {
        cancelElection();
        throw new AlreadyClosedException();
      }
      log.debug("I am the new leader, publishing as active: " + leaderProps.getCoreUrl() + " " + shardId);
