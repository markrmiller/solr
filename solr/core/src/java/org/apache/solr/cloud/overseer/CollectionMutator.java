...
public class CollectionMutator {
  ...
  public ClusterState deleteShard(final ClusterState clusterState, ZkNodeProps message) {
    ...
    newSlices.get(sliceId).put("remove", "true");
  ...
  public ClusterState modifyCollection(final ClusterState clusterState, ZkNodeProps message) {
    ...
    for (Object theEntry : message.entrySet()) {
      ...
      if (((String) prop).startsWith(CollectionAdminRequest.PROPERTY_PREFIX)) {
        hasAnyOps = true;
        if (entry.getValue() == null) {
          m.remove(prop);
        } else {
          m.put((String) prop, entry.getValue());
        }
      }
    }
    ...
    return clusterState.copyWith(coll.getName(),
        new DocCollection(coll.getName(), coll.getSlicesMap(), m, coll.getRouter(), coll.getZNodeVersion(), coll.getStateUpdates(), coll.getStateUpdatesZkVersion()));
  }
