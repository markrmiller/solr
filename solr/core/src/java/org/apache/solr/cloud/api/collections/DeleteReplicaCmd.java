...
public class DeleteReplicaCmd implements Cmd {
  ...
  public AddReplicaCmd.Response call(ClusterState clusterState, ZkNodeProps message, @SuppressWarnings({"rawtypes"})NamedList results) throws Exception {
    ...
    AddReplicaCmd.Response response = deleteReplica(clusterState, message, shardHandler, shardRequestTracker, results, collectionName, shardId);
    return response;
  }
...
//    }
    ...
    rep.put("replica", replicaName);
    rep.put("collection", replica.getCollection());
    rep.put("node", replica.getNodeName());
