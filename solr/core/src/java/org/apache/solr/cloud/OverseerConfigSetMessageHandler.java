...
public class OverseerConfigSetMessageHandler implements OverseerMessageHandler {
  ...
  private Map<String, Object> getNewProperties(ZkNodeProps message) {
    ...
    for (Object entry : message.getProperties().entrySet()) {
      if (((Map.Entry<String,Object>) entry).getKey().startsWith(PROPERTY_PREFIX + ".")) {
        if (properties == null) {
          properties = new HashMap<String, Object>();
        }
        properties.put(((Map.Entry<String,Object>) entry).getKey().substring((PROPERTY_PREFIX + ".").length()),
