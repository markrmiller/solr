...
public class SliceMutator {
  ...
  public ClusterState addReplica(ClusterState clusterState, ZkNodeProps message, Overseer overseer) {
    ...
    DocCollection collection = clusterState.getCollectionRef(coll).get(false);
    ...
    Replica replica = new Replica(coreName,
        Utils.makeNonNullMap("id", String.valueOf(id),
                    ...
                    ZkStateReader.REPLICA_TYPE, message.get(ZkStateReader.REPLICA_TYPE)), coll, collection.getId(), slice);
  ...
  public ClusterState removeReplica(ClusterState clusterState, ZkNodeProps message) {
    ...
    for (Slice slice : coll.getSlices()) {
      ...
      if (replica != null) {
        ...
        Replica removeReplica = new Replica(coreName, props, collection, coll.getId(), slice.getName());
        newReplicas.put(coreName, removeReplica);
        slice = new Slice(slice.getName(), newReplicas, slice, collection, coll.getId());
      }
  ...
  public ClusterState setShardLeader(ClusterState clusterState, ZkNodeProps message) {
    ...
    slice = new Slice(slice.getName(), newReplicas, slice, collectionName, coll.getId());
  ...
  public ClusterState updateShardState(ClusterState clusterState, ZkNodeProps message) {
    ...
    for (Object key : message.keySet()) {
      ...
      Slice slice = collection.getSlice((String) key);
      if (slice == null) {
        throw new RuntimeException("Overseer.updateShardState unknown slice: " + collectionName + " slice: " + key);
      }
      if (log.isDebugEnabled()) log.debug("Update shard state " + key + " to " + message.getStr((String) key));
      Map<String, Object> props = slice.shallowCopy();

      if (Slice.State.getState(message.getStr((String) key)) == Slice.State.ACTIVE) {
        props.remove(Slice.PARENT);
        props.remove("shard_parent_node");
        props.remove("shard_parent_zk_session");
      }
      props.put(ZkStateReader.STATE_PROP, message.getStr((String) key));
  ...
  public static DocCollection updateReplica(DocCollection collection, final String shard, final Replica replica) {
    ...
    Slice newSlice = new Slice(slice.getName(), replicasCopy, slice, collection.getName(), collection.getId());
    if (log.isDebugEnabled()) {
