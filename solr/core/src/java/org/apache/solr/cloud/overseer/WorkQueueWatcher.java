...
public class WorkQueueWatcher extends QueueWatcher {
 ...
 // private final ReentrantLock ourLock = new ReentrantLock(true);
  ...
  public void start(boolean weAreReplacement) throws KeeperException, InterruptedException {
    ...
    Queue startItems = getItems(true);
  ...
  protected Set<Integer> processQueueItems(Queue<String> items, boolean onStart, boolean weAreReplacement) {
    ...
    data.forEach((key, value) -> {
      Map<Integer,Map<Integer,Integer>> replicaStates = new NonBlockingHashMap<>();
      Map<Integer,List<ZkStateWriter.StateUpdate>> sliceStates = new NonBlockingHashMap<>();
      ...
      switch (overseerAction) {
        case STATE:
          for (Object theEntry : message.getProperties().entrySet()) {
                    ...
                    if (docCollection != null) {
                      ...
                      for (Replica replica : replicas) {
                        if (replica.getNodeName().equals(nodeName)) {
                          ...
                          Map<Integer,Integer> updates = replicaStates.computeIfAbsent(replica.getCollectionId(), k -> new NonBlockingHashMap<>());
  ...
  protected void doneWithCurrentQueue( Set<Integer> collIds) {
