...
public class CollapsingQParserPlugin extends QParserPlugin {
  ...
  */
  ...
  private static class OrdFieldValueCollector extends DelegatingCollector {
    ...
    private final OrdFieldValueStrategy collapseStrategy;
    ...
    public OrdFieldValueCollector(int maxDoc,
      ...
      if (null != sortSpec) {
        this.collapseStrategy = new OrdSortSpecStrategy(maxDoc, nullPolicy, valueCount, groupHeadSelector, needsScores4Collapsing, this.needsScores, boostDocs, sortSpec, searcher, collapseValues);
      } else if (funcQuery != null) {
        this.collapseStrategy =  new OrdValueSourceStrategy(maxDoc, nullPolicy, valueCount, groupHeadSelector, needsScores4Collapsing, this.needsScores, boostDocs, funcQuery, searcher, collapseValues);
      } else {
  ...
  private static class CollectorFactory {
    ...
    private final static EnumSet<NumberType> NUMERIC_COLLAPSIBLE_TYPES = EnumSet.of(NumberType.INTEGER,
                                                                                    NumberType.FLOAT);
    private static boolean isNumericCollapsible(FieldType collapseFieldType) {
      return NUMERIC_COLLAPSIBLE_TYPES.contains(collapseFieldType.getNumberType());
    }

    public static DelegatingCollector getCollector(String collapseField, GroupHeadSelector groupHeadSelector, SortSpec sortSpec, int nullPolicy, String hint,
        boolean needsScores4Collapsing, boolean needsScores, int size, IntIntHashMap boostDocs, SolrIndexSearcher searcher) throws IOException {
  ...
  */
  ...
  private static int numNotNull(final Object... args) {
    int r = 0;
