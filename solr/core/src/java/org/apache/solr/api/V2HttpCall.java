...
public class V2HttpCall extends SolrCall {
  ...
  public V2HttpCall(SolrDispatchFilter solrDispatchFilter, String path, HttpServletRequest request, HttpServletResponse response) throws Exception {
    ...
    Api solrApi = null;
    reqPath = reqPath.substring(7);//strip off '/____v2'
    HashMap<String, String> parts = new HashMap<>();
    List<String> pieces = getPathSegments(reqPath);
    try {

      String prefix;

      if (pieces.size() == 1 || (pieces.size() == 2 && reqPath.endsWith(CommonParams.INTROSPECT))) {
        api = new MyApi();
        this.solrReq = SolrRequestParsers.DEFAULT.parse(null, reqPath, req);
        solrReq.getContext().put(CoreContainer.class.getName(), cores);
        solrReq.getContext().put(CommonParams.PATH, reqPath);
        this.action = ADMIN;
        this.requestType = AuthorizationContext.RequestType.ADMIN;
        this.pieces = pieces;
        this.parts = Collections.EMPTY_MAP;
        this.path = reqPath;
        this.coreUrl = null;
        return;
      } else {
      ...
      if (knownPrefixes.contains(prefix)) {
        solrApi = getApiInfo(cores.getRequestHandlers(), reqPath, request.getMethod(), reqPath, parts);
        if (solrApi != null) {
      ...
      if (("c".equals(prefix) || "collections".equals(prefix)) && pieces.size() > 1) {
        ...
        if (collection == null) {
          ...
          core = getCoreByCollection(cores, collection.getName(), isPreferLeader);
          if (core == null) {
            ...
            if (callAction == REMOTEQUERY) {
              solrCoreUrl = solrCoreUrl.replace("/solr/", "/solr/____v2/c/");
              reqPath.substring(prefix.length() + collection.getName().length() + 2);
              api = solrApi;
      ...
      Api apiInfo = getApiInfo(core.getRequestHandlers(), reqPath, req.getMethod(), reqPath, parts);
      if (isCompositeApi && apiInfo instanceof CompositeApi) {
        ((CompositeApi) solrApi).add(apiInfo);
      } else {
        solrApi = apiInfo == null ? solrApi : apiInfo;
      }
      this.solrReq = parseRequest(core, reqPath);
  ...
  SolrDispatchFilter.Action call() throws IOException {
    ...
    String abortErrorMessage = solrDispatchFilter.getAbortErrorMessage();
    ...
    try {
      ...
      switch (action) {
        ...
        case REMOTEQUERY:
          ...
          SolrDispatchFilter.Action a = remoteQuery(req, response, coreUrl + path, solrDispatchFilter.getHttpClient());
          return a;
        case PROCESS:
