...
public class JettySolrRunner implements Closeable {
  ...
  public JettySolrRunner(String solrHome, Properties nodeProperties, JettyConfig config) {
    ...
    scheduler = new ScheduledExecutorScheduler("jetty-scheduler", true);
...
//    }
    ...
    //  NegotiatingServerConnectionFactory.checkProtocolNegotiationAvailable();
      ...
      // HTTP/2 Connector
      if (ssl == null) {
        connector = new ServerConnector(server, httpFactory, h2);
      } else {
        connector = new ServerConnector(server, ssl, alpn, h2, httpFactory);
